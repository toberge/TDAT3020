...
app.post('/users', (req, res) => {
  try {
    ...
    if (users.find(user => user.username === username)) // unless it already exists!
...
app.post('/login', (req, res) => {
  try {
    const { username, clientHash } = req.body;
    const { serverHash, serverSalt } = users.find(user => user.username === username); // simple & will break
    // hash it!
